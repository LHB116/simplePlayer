...
// opaque 用户在注册回调函数时指定的参数
// stream 音频数据缓冲区地址，将解码后的音频数据填入此缓冲区
// len    音频数据缓冲区大小，单位字节
...
static void sdl_audio_callback(void* opaque, Uint8* stream, int len)
{
    ...
    int64_t audio_callback_time = av_gettime_relative();  // while可能产生延迟

    while (len > 0) // 输入参数len等于is->audio_hw_buf_size，是audio_open()中申请到的SDL音频缓冲区大小
    {
        // 位置索引大于数据size，数据不足；否则还剩数据未处理
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (ret < 0)
            {
                if (ret == AVERROR_EOF)
                {
                    av_log(NULL, AV_LOG_INFO, "audio avcodec_receive_frame(): the decoder has been fully flushed\n");
                    avcodec_flush_buffers(p_codec_ctx);
                    return 0;
                }
                else if (ret == AVERROR(EAGAIN))
                {
                    //av_log(NULL, AV_LOG_INFO, "audio avcodec_receive_frame(): output is not available in this state - "
        ...
        // packet_queue中第一个总是flush_pkt（data=NULL，初始化时放入的）。
        // 每次seek操作会插入flush_pkt，更新serial，开启新的播放序列
        if (pkt.data == NULL)
        {
...
static int open_audio_playing(void *arg)
{
    ...
    wanted_spec.freq = is->p_acodec_ctx->sample_rate;   // 采样率   44100
    wanted_spec.format = AUDIO_S16SYS;                  // S表带符号，16是采样深度，SYS表采用系统字节序
    wanted_spec.channels = is->p_acodec_ctx->channels;  // 声音通道数  2
    ...
    if (audio_dev = SDL_OpenAudio(&wanted_spec, NULL) < 0)
    {
        av_log(NULL, AV_LOG_ERROR, "SDL_OpenAudio() failed: %s\n", SDL_GetError());
        return -1;
    }
    //if (audio_dev = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &actual_spec, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_CHANNELS_CHANGE) < 0)
    //{
    //    av_log(NULL, AV_LOG_ERROR, "SDL_OpenAudio() failed: %s\n", SDL_GetError());
    //    return -1;
    //} 
    ...
    // 然后送再写入SDL音频缓冲区
    // 
    //printf("freq, %d, %d\n", wanted_spec.freq, actual_spec.freq);
    //printf("channels, %d, %d\n", wanted_spec.channels, actual_spec.channels);
    //printf("size, %d, %d\n", wanted_spec.size, actual_spec.size);
    //exit(0);

    is->audio_param_tgt.fmt = AV_SAMPLE_FMT_S16;
    is->audio_param_tgt.freq = wanted_spec.freq;
    is->audio_param_tgt.channel_layout = av_get_default_channel_layout(wanted_spec.channels);;
    is->audio_param_tgt.channels = wanted_spec.channels;
    is->audio_param_tgt.frame_size = av_samples_get_buffer_size(NULL, wanted_spec.channels, 1, is->audio_param_tgt.fmt, 1);
    //printf("11, %d\n", actual_spec.freq);
    //exit(0);
    is->audio_param_tgt.bytes_per_sec = av_samples_get_buffer_size(NULL, wanted_spec.channels, wanted_spec.freq, is->audio_param_tgt.fmt, 1);
    ...
    is->audio_hw_buf_size = 3768;   // SDL音频缓冲区大小 actual_spec.size
    is->audio_frm_size = 0;
    is->audio_cp_index = 0;
...
static int open_audio_playing_ffplay(void* arg)
{
    player_stat_t* is = (player_stat_t*)arg;
    ...
    int64_t wanted_channel_layout = is->p_acodec_ctx->channel_layout;
    int wanted_nb_channels = is->p_acodec_ctx->channels, wanted_sample_rate = is->p_acodec_ctx->sample_rate;
    ...
    */
    ...
    is->audio_param_tgt.fmt = AV_SAMPLE_FMT_S16;
    is->audio_param_tgt.freq = spec.freq;
    is->audio_param_tgt.channel_layout = wanted_channel_layout;
    is->audio_param_tgt.channels = spec.channels;
    /* audio_hw_params->frame_size这里只是计算一个采样点占用的字节数 */
    is->audio_param_tgt.frame_size = av_samples_get_buffer_size(NULL, is->audio_param_tgt.channels,
        1, is->audio_param_tgt.fmt, 1);
    // bytes_per_sec就说一秒钟音频字节数=采样率 * 声道数 * 采样格式大小（bit） / 8
    is->audio_param_tgt.bytes_per_sec = av_samples_get_buffer_size(NULL, is->audio_param_tgt.channels,
        is->audio_param_tgt.freq,
        is->audio_param_tgt.fmt, 1);
    if (is->audio_param_tgt.bytes_per_sec <= 0 || is->audio_param_tgt.frame_size <= 0) {
        av_log(NULL, AV_LOG_ERROR, "av_samples_get_buffer_size failed\n");
        return -1;
    }
    ...
    is->audio_param_src = is->audio_param_tgt;
    is->audio_hw_buf_size = spec.size;   // SDL音频缓冲区大小 actual_spec.size
    is->audio_frm_size = 0;
    is->audio_cp_index = 0;
    ...
    // 音频一帧大小 = 一帧采样点数（AAC为1024） * 声道数 * 采样格式大小（bit） / 8
    // spec.size取的是两帧数据大小，一帧就是1024个采样点， 1024*2*2 * 2 = 8192字节
