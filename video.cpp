...
static int video_decode_frame(AVCodecContext* p_codec_ctx, packet_queue_t* p_pkt_queue, AVFrame* frame)
{
    ...
    while (1)
    {
        ...
        {
            // 复位解码器内部状态/刷新内部缓冲区。
            //printf("复位解码器内部状态/刷新内部缓冲区。\n");
            avcodec_flush_buffers(p_codec_ctx);
        }
        else
        {
            // 将packet发送给解码器, 发送packet的顺序是按dts递增的顺序，如IPBBPBB
...
static int video_decode_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        duration = (frame_rate.num && frame_rate.den ? av_q2d({frame_rate.den, frame_rate.num}) : 0);   // 当前帧播放时长
...
static int open_video_playing(void *arg)
{
    ...
    // 计算下如果不按照视频原始分辨率计算时，按照原比例播放窗口宽高
    int display_width = is->p_vcodec_ctx->width, display_height = is->p_vcodec_ctx->height;

    if (!OriginalSize) {
        int scr_width = WINDOW_W;
        int scr_height = WINDOW_H;
        float aspect_ratio = (float)is->p_vcodec_ctx->width / (float)is->p_vcodec_ctx->height;
        display_height = scr_height;
        display_width = lrint(display_height * aspect_ratio) & ~1; // 结果是奇数要减一
        if (display_width > scr_width)
        {
            display_width = scr_width;
            display_height = lrint(display_width / aspect_ratio) & ~1;
        }
    }
    ...
    is->img_convert_ctx = sws_getContext(is->p_vcodec_ctx->width,   // src width
        ...
        display_width,   // dst width  is->p_vcodec_ctx->width
        display_height,  // dst height  is->p_vcodec_ctx->height
    ...
    is->sdl_video.rect.w = display_width;
    is->sdl_video.rect.h = display_height;
    ...
    is->sdl_video.window = SDL_CreateWindow("simple ffplayer",
        ...
        display_width,  // is->sdl_video.rect.w
        display_height,  // is->sdl_video.rect.h
        SDL_WINDOW_OPENGL
    );
    ...
    // 2. 创建SDL_Renderer
    //    SDL_Renderer：渲染器
    is->sdl_video.renderer = SDL_CreateRenderer(is->sdl_video.window, -1,
    ...
    SDL_SetRenderDrawColor(is->sdl_video.renderer, 0xff, 0xc0, 0xcb, 0xff);
    SDL_RenderClear(is->sdl_video.renderer);
